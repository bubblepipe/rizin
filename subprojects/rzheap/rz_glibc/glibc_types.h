...
typedef struct rz_glibc_config_t {
	...
	RzGlibcChunkLayout chunk;
	MallocStateLayout malloc_state;
	RzGlibcHeapInfoLayout heap_info;
	RzGlibcTcacheLayout tcache;
	RzGlibcTcacheEntryLayout tcache_entry;
...
static inline void rz_glibc_config_init(RzGlibcConfig *config, ut8 ptr_size,
	...
	config->min_chunk_size = ptr_size * 4;
	config->malloc_alignment = ptr_size * 2;
	config->chunk_hdr_size = ptr_size * 2;
	config->heap_page_size = (ptr_size == 4) ? 0x22000 : 0x21000;
	config->mmap_align = (ptr_size == 8) ? 0x18 : 0x14;
	config->mmap_offset = 0x8;
	config->tcache_hdr_size = 0x10;
	config->tcache_chunk_extra = (ptr_size == 8) ? 0x10 : 0x0;
}
...
static inline ut64 rz_glibc_chunk_size(const RzGlibcChunk *chunk, const RzGlibcConfig *config) {
	ut64 mask = ~(ut64)(config->malloc_alignment - 1);
	return chunk->size & mask;
}

static inline bool rz_glibc_chunk_prev_inuse(const RzGlibcChunk *chunk) {
	return (chunk->size & RZ_GLIBC_PREV_INUSE) != 0;
}

static inline bool rz_glibc_chunk_is_mmapped(const RzGlibcChunk *chunk) {
	return (chunk->size & RZ_GLIBC_IS_MMAPPED) != 0;
}

static inline bool rz_glibc_chunk_non_main_arena(const RzGlibcChunk *chunk) {
	return (chunk->size & RZ_GLIBC_NON_MAIN_ARENA) != 0;
}

static inline ut64 rz_glibc_chunk_to_mem(ut64 chunk_addr, const RzGlibcConfig *config) {
	return chunk_addr + config->chunk_hdr_size;
}

static inline ut64 rz_glibc_mem_to_chunk(ut64 mem_addr, const RzGlibcConfig *config) {
	return mem_addr - config->chunk_hdr_size;
}

static inline ut64 rz_glibc_next_chunk(ut64 chunk_addr, const RzGlibcChunk *chunk,
	const RzGlibcConfig *config) {
	return chunk_addr + rz_glibc_chunk_size(chunk, config);
}

static inline ut64 rz_glibc_prev_chunk(ut64 chunk_addr, const RzGlibcChunk *chunk) {
	return chunk_addr - chunk->prev_size;
}
