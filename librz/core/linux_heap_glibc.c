...
void print_heap_chunk_simple(RzCore *core, ut64 chunk, const char *status, PJ *pj, const RzGlibcConfig *config);
static void rz_heap_get_brks(RzCore *core, ut64 *brk_start, ut64 *brk_end);
RZ_API bool rz_heap_update_main_arena(RzCore *core, ut64 m_arena, MallocState *main_arena, const RzGlibcConfig *config);
RZ_IPI bool rz_heap_is_arena(RzCore *core, ut64 m_arena, ut64 m_state, const RzGlibcConfig *config);
RZ_IPI bool rz_heap_is_tcache(RzCore *core);
RZ_API RzHeapChunkSimple *rz_heap_chunk_internal(RzCore *core, ut64 addr, const RzGlibcConfig *config);
RZ_API void rz_heap_bin_free(RzHeapBin *bin);
RZ_IPI void rz_heap_chunk_free(RzHeapChunkListItem *item);
RZ_IPI void rz_arena_list_free(RzArenaListItem *item);
...
RZ_IPI RzList *rz_heap_arenas_list_internal(RzCore *core, ut64 m_arena, MallocState *main_arena, const RzGlibcConfig *config);
RZ_IPI RzList *rz_heap_chunks_list_internal(RzCore *core, MallocState *main_arena, ut64 m_arena, ut64 m_state, bool show_unallocated, const RzGlibcConfig *config);
RZ_API RzHeapBin *rz_heap_bin_content_internal(RzCore *core, MallocState *arena, int bin_num, ut64 m_arena, const RzGlibcConfig *config);
RZ_API RzHeapBin *rz_heap_fastbin_content_internal(RzCore *core, MallocState *arena, int bin_num, const RzGlibcConfig *config);
RZ_API RzList *rz_heap_tcache_content_internal(RzCore *core, ut64 arena_base, const RzGlibcConfig *config);
RZ_API MallocState *rz_heap_get_arena(RzCore *core, ut64 m_state);
static inline bool init_glibc_config(RzCore *core, RzGlibcConfig *config);
...
static inline ut64 rz_heap_get_next_pointer(RzCore *core, ut64 pos, ut64 next, const RzGlibcConfig *config) {
	if (!config) {
		return next;
	}
...
static RZ_BORROW RzList /*<RzList *>*/ *fill_tcache_entries(RzCore *core, const RzGlibcTcache *tcache,
...
static void print_tcache(RzCore *core, RzList /*<RzList *>*/ *bins, PJ *pj, const ut64 tid, const RzGlibcConfig *config) {
...
static void print_arena_stats(RzCore *core, ut64 m_arena, MallocState *main_arena, ut64 global_max_fast, int format, const RzGlibcConfig *config) {
	size_t i, j, k, start;
...
RZ_API bool rz_heap_resolve_main_arena(RzCore *core, ut64 *m_arena) {
	...
	if (main_arena_sym != UT64_MAX) {
		rz_heap_update_main_arena(core, main_arena_sym, ta, &config);
	...
	while (addr_srch < libc_addr_end) {
		rz_heap_update_main_arena(core, addr_srch, ta, &config);
...
static bool parse_tcache_from_addr(RzCore *core, const ut64 tls_addr, const ut64 tid, const RzGlibcConfig *config) {
	RzDebugMap *map;
	RzListIter *iter;
	ut8 tcache_addr[8] = { 0 };
	if (!config) {
	...
	rz_list_foreach (core->dbg->maps, iter, map) {
		...
		if (map->size != config->heap_page_size || map->perm != RZ_PERM_RW) {
			if (strcmp(core->dbg->arch, "x86") || map->size != config->heap_page_size) {
				continue;
			}
		}
...
static bool parse_tls_data(RzCore *core, RZ_NONNULL RzDebugPid *th, ut64 tid, const RzGlibcConfig *config) {
...
static void resolve_tcache_perthread(RZ_NONNULL RzCore *core, const RzGlibcConfig *config) {
	RzDebugPid *th;
	RzListIter *it;
	ut64 tid = 1;
...
RZ_API RZ_OWN bool resolve_heap_tcache(RZ_NONNULL RzCore *core, ut64 arena_base, const RzGlibcConfig *config) {
	RzDebug *dbg = core->dbg;
...
void print_heap_chunk(RzCore *core, ut64 chunk, const RzGlibcConfig *config) {
	RzConsPrintablePalette *pal = &rz_cons_singleton()->context->pal;
	if (!config) {
		return;
	}
	...
	if (!have_full) {
		ut8 ptr_buf[16] = { 0 };
		if (rz_io_read_at_mapped(core->io, rz_glibc_chunk_to_mem(chunk, config), ptr_buf, config->ptr_size * 2)) {
			if (config->ptr_size == 8) {
				cnk.fd = rz_read_le64(ptr_buf);
				cnk.bk = rz_read_le64(ptr_buf + 8);
			} else {
				cnk.fd = rz_read_le32(ptr_buf);
				cnk.bk = rz_read_le32(ptr_buf + 4);
			}
		}
	}
	...
	if (size > config->ptr_size * 128) {
		if (!have_full) {
			ut8 ptr_buf[16] = { 0 };
			if (rz_io_read_at_mapped(core->io, rz_glibc_chunk_to_mem(chunk, config) + (config->ptr_size * 2), ptr_buf, config->ptr_size * 2)) {
				if (config->ptr_size == 8) {
					cnk.fd_nextsize = rz_read_le64(ptr_buf);
					cnk.bk_nextsize = rz_read_le64(ptr_buf + 8);
				} else {
					cnk.fd_nextsize = rz_read_le32(ptr_buf);
					cnk.bk_nextsize = rz_read_le32(ptr_buf + 4);
				}
			}
		}
		PRINT_GA(",\n  fd-nextsize = ");
...
RZ_IPI bool rz_heap_is_arena(RzCore *core, ut64 m_arena, ut64 m_state, const RzGlibcConfig *config) {
	...
	MallocState ta_storage = { 0 };
	MallocState *ta = &ta_storage;
	if (!rz_heap_update_main_arena(core, m_arena, ta, config)) {
		return false;
	}
	if (ta->next == m_state) {
		return true;
	...
	while (ta->next != UT64_MAX && ta->next != m_arena) {
		if (!rz_heap_update_main_arena(core, ta->next, ta, config)) {
			return false;
		}
		if (ta->next == m_state) {
			return true;
...
static void print_heap_bin(RzCore *core, ut64 m_arena, MallocState *main_arena, const char *input, bool print_graph, const RzGlibcConfig *config) {
...
void print_heap_fastbin(RzCore *core, ut64 m_arena, MallocState *main_arena, ut64 global_max_fast, const char *input, bool main_arena_only, PJ *pj, const RzGlibcConfig *config) {
	RzConsPrintablePalette *pal = &rz_cons_singleton()->context->pal;
	const ut8 ptr_size = config ? config->ptr_size : rz_heap_ptr_size(core);
...
// 		return NULL;
// 	}
RZ_API RzList /*<RzHeapBin *>*/ *rz_heap_tcache_content_internal(RzCore *core, ut64 arena_base, const RzGlibcConfig *config) {
	...
	if (arena_base == m_arena) {
		...
		if (!rz_heap_update_main_arena(core, arena_base, arena, config) || !arena->attached_threads) {
			return NULL;
		}
	}

	RzGlibcTcache tcache;
...
static void print_tcache_content(RzCore *core, ut64 arena_base, ut64 main_arena_base, PJ *pj, const RzGlibcConfig *config) {
	RzConsPrintablePalette *pal = &rz_cons_singleton()->context->pal;
...
void print_inst_minfo(RzCore *core, const RzGlibcHeapInfo *heap_info, ut64 hinfo) {
	RzConsPrintablePalette *pal = &rz_cons_singleton()->context->pal;
...
void print_malloc_info(RzCore *core, ut64 m_state, ut64 malloc_state, const RzGlibcConfig *config) {
	ut64 h_info;
	RzConsPrintablePalette *pal = &rz_cons_singleton()->context->pal;
	if (malloc_state == m_state) {
		...
		while (heap_info.prev != 0x0 && heap_info.prev != UT64_MAX) {
			if (!rz_heap_update_main_arena(core, malloc_state, ms, config)) {
				return;
			}
 ...
 */
static int print_bin_content(RzCore *core, MallocState *main_arena, int bin_num, PJ *pj, ut64 m_arena, const RzGlibcConfig *config) {
	RzListIter *iter;
	RzHeapChunkListItem *pos;
 ...
 */
static void print_unsortedbin_description(RzCore *core, ut64 m_arena, MallocState *main_arena, PJ *pj, const RzGlibcConfig *config) {
	RzConsPrintablePalette *pal = &rz_cons_singleton()->context->pal;
	if (!pj) {
 ...
 */
static void print_smallbin_description(RzCore *core, ut64 m_arena, MallocState *main_arena, PJ *pj, const RzGlibcConfig *config) {
	RzConsPrintablePalette *pal = &rz_cons_singleton()->context->pal;
	if (!pj) {
 ...
 * \param main_arena Pointer to MallocState struct for the arena in which bins are
 */
static void print_largebin_description(RzCore *core, ut64 m_arena, MallocState *main_arena, PJ *pj, const RzGlibcConfig *config) {
	RzConsPrintablePalette *pal = &rz_cons_singleton()->context->pal;
	if (!pj) {
 ...
 */
static void print_main_arena_bins(RzCore *core, ut64 m_arena, MallocState *main_arena, ut64 main_arena_base, ut64 global_max_fast, RzHeapBinType format, bool json, const RzGlibcConfig *config) {
 ...
 */
RZ_IPI RzList /*<RzArenaListItem *>*/ *rz_heap_arenas_list_internal(RzCore *core, ut64 m_arena, MallocState *main_arena, const RzGlibcConfig *config) {
	...
	if (main_arena->next != m_arena) {
		...
		while (rz_heap_is_arena(core, m_arena, ta->next, config) && ta->next != m_arena) {
			...
			if (!rz_heap_update_main_arena(core, ta_addr, ta, config)) {
				free(ta);
				return arena_list;
			}
			// thread arenas
...
RZ_IPI RzList /*<RzHeapChunkListItem *>*/ *rz_heap_chunks_list_internal(RzCore *core, MallocState *main_arena,
	ut64 m_arena, ut64 m_state, bool top_chunk, const RzGlibcConfig *config) {
	...
	after_tcache:
		...
		if (!rz_glibc_read_chunk(core->io, next_chunk, &cnk, config)) {
			if (next_chunk == main_arena->top) {
				...
				cnk.size = size_raw;
				RzGlibcChunk tmp_next = { .size = size_raw };
...
RZ_IPI RzCmdStatus rz_cmd_arena_print_handler(RzCore *core, int argc, const char **argv) {
	...
	if (!rz_heap_update_main_arena(core, m_arena, main_arena, &config)) {
		return RZ_CMD_STATUS_ERROR;
	}
 ...
 */
RZ_IPI RzList /*<RzArenaListItem *>*/ *rz_heap_arena_list(RzCore *core) {
	ut64 m_arena;
	if (!rz_heap_resolve_main_arena(core, &m_arena)) {
...
RZ_API RzHeapChunkSimple *rz_heap_chunk_internal(RzCore *core, ut64 addr, const RzGlibcConfig *config) {
	rz_return_val_if_fail(core && config, NULL);
	RzGlibcChunk chunk;
	if (!rz_glibc_read_chunk(core->io, addr, &chunk, config)) {
		return NULL;
	}
...
RZ_API bool rz_heap_write_chunk(RzCore *core, RzHeapChunkSimple *chunk_simple) {
	...
	if (config.ptr_size == 8) {
		rz_write_ble64(buf + config.chunk.prev_size, chunk.prev_size, be);
		rz_write_ble64(buf + config.chunk.size, chunk.size, be);
		rz_write_ble64(buf + config.chunk.fd, chunk.fd, be);
		rz_write_ble64(buf + config.chunk.bk, chunk.bk, be);
		rz_write_ble64(buf + config.chunk.fd_nextsize, chunk.fd_nextsize, be);
		rz_write_ble64(buf + config.chunk.bk_nextsize, chunk.bk_nextsize, be);
	} else {
		rz_write_ble32(buf + config.chunk.prev_size, (ut32)chunk.prev_size, be);
		rz_write_ble32(buf + config.chunk.size, (ut32)chunk.size, be);
		rz_write_ble32(buf + config.chunk.fd, (ut32)chunk.fd, be);
		rz_write_ble32(buf + config.chunk.bk, (ut32)chunk.bk, be);
		rz_write_ble32(buf + config.chunk.fd_nextsize, (ut32)chunk.fd_nextsize, be);
		rz_write_ble32(buf + config.chunk.bk_nextsize, (ut32)chunk.bk_nextsize, be);
	}
